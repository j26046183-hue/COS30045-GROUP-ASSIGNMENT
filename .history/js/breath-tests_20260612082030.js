...
Promise.all([
    ...
    // Line chart state filter — all states
    ...
    states.forEach(s => lineStateFilter.append("option").attr("value", s).text(s));

    // Donut state filter — only states with location data
    const donutStates = ["ACT", "NSW", "VIC"];
    const donutStateFilter = d3.select("#breath-donut-state-filter");
    donutStateFilter.selectAll("option:not([value='all'])").remove();
    donutStates.forEach(s => donutStateFilter.append("option").attr("value", s).text(s));

    // Bar state filter — only states with age data
    const barStates = ["ACT", "NSW", "VIC"];
    const barStateFilter = d3.select("#breath-bar-state-filter");
    barStateFilter.selectAll("option:not([value='all'])").remove();
    barStates.forEach(s => barStateFilter.append("option").attr("value", s).text(s));
    ...
    d3.select("#breath-line-state-filter").on("change", applyBreathFilters);
    d3.select("#breath-donut-state-filter").on("change", updateDonutOnly);
    d3.select("#breath-bar-state-filter").on("change", updateBarOnly);
...
function applyBreathFilters() {
    const selectedState = d3.select("#breath-line-state-filter").property("value") || "all";
...
function updateBarOnly() {
    const selectedState = d3.select("#breath-bar-state-filter").property("value") || "all";
