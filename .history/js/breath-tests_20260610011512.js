...
// =========================================================================
// CHART 2: TOTAL BREATH TESTS — SCATTER PLOT LAYOUT
// =========================================================================
function drawBreathScatter(data) {
    d3.select("#breath-bar-chart").selectAll("*").remove(); // Reuses your breath-bar-chart container ID natively
    if (data.length === 0) return;
    ...
    nodes.append("circle")
        ...
        .on("mouseover", function(event, d) {
            ...
            breathTooltip.style("opacity", 1)
                .html(`<strong>${d.STATE}</strong><br>Total Tests: ${d.COUNT.toLocaleString()}`);
        })
        .on("mousemove", function(event) {
            breathTooltip.style("left", (event.pageX + 14) + "px").style("top", (event.pageY - 32) + "px");
        })
...
function drawBreathDonut(data) {
    d3.select("#breath-actions-chart").selectAll("*").remove(); // Reuses your breath-actions-chart container ID natively
    if (data.length === 0) return;
    ...
    arcs.append("path")
        ...
        .on("mouseover", function(event, d) {
            ...
            breathTooltip.style("opacity", 1)
                .html(`<strong>${breathLabels[d.data.action]}</strong><br>Volume Count: ${d.data.value.toLocaleString()}<br>Proportion: ${pct}%`);
        })
        .on("mousemove", function(event) {
            breathTooltip.style("left", (event.pageX + 14) + "px").style("top", (event.pageY - 32) + "px");
        })
