...
Promise.all([
    ...
    const years = [...new Set(historical.map(d => d.YEAR))].sort();
    const yearFilter = d3.select("#fines-year-filter");
    ...
    d3.select("#fines-year-filter").on("change", applyFinesFilters);
...
function applyFinesFilters() {
    ...
    const selectedYear = d3.select("#fines-year-filter").property("value");
    ...
    if (selectedYear !== "all") filteredAge = filteredAge.filter(d => d.YEAR === +selectedYear);
    ...
    if (selectedYear !== "all") filteredLocation = filteredLocation.filter(d => d.YEAR === +selectedYear);
