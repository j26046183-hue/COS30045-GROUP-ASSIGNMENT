...
// Mapped exactly to support your unique CSV structures ("0-16", "Unknown", etc.)
const breathAgeOrder = ["0-16", "Under 17", "17-25", "26-39", "40-64", "65 and over", "Unknown", "Age Group Not Specified", "All ages"];
const breathAgeColors = ["#f43f5e", "#f43f5e", "#bfdbfe", "#60a5fa", "#2563eb", "#1d4ed8", "#a855f7", "#a855f7", "#ec4899"];
...
d3.csv("data/breath_2023_2024.csv").then(function(rawCSV) {
    ...
    rawCSV.forEach(d => {
        ...
        // Clean up locations and handle rows missing granular values (2008-2022 block)
        if (!d.LOCATION || d.LOCATION.trim() === "") {
            d.LOCATION = "All regions";
        } else {
            d.LOCATION = d.LOCATION.trim();
            if (d.LOCATION.toLowerCase() === "unknown") {
                d.LOCATION = "Location Not Specified";
            }
        }

        // Standardize age missing fields
        if (!d.AGE_GROUP || d.AGE_GROUP.trim() === "") {
    ...
    // Dynamically fetch and pop options across full chronological line
...
function applyBreathFilters() {
    ...
    // Aggregate absolute totals for Key KPI cards safely across mixed schemas
    // Filter to avoid duplicating totals from granular entries vs master totals in same files
    const blockKPI = filteredData.filter(d => d.LOCATION.toLowerCase() === "all regions" || d.LOCATION === "Location Not Specified");
    
    // If no master rows present (e.g. state has only granular rows), take full sum
    const totalCount   = blockKPI.length ? d3.sum(blockKPI, d => d.COUNT) : d3.sum(filteredData, d => d.COUNT);
    const totalCharges = blockKPI.length ? d3.sum(blockKPI, d => d["TOTAL CHARGES"]) : d3.sum(filteredData, d => d["TOTAL CHARGES"]);
    const totalArrests = blockKPI.length ? d3.sum(blockKPI, d => d["TOTAL ARRESTS"]) : d3.sum(filteredData, d => d["TOTAL ARRESTS"]);
...
function drawBreathHistorical(data) {
    ...
    // Isolate historical baseline timelines
...
function updateDonutOnly() {
    ...
    if (selectedYear !== "all") {
        ...
        // Fallback filter matching checks if state includes granular data lines (2023-2024)
        const hasModernData = filtered.some(d => (d.YEAR === 2023 || d.YEAR === 2024) && d.LOCATION.toLowerCase() !== "all regions" && d.LOCATION !== "Location Not Specified");
        if (hasModernData) {
    ...
    // Hide massive meta-block summaries if separate deep location details exist
...
function updateBarOnly() {
    ...
    if (selectedYear !== "all") {
        ...
        const hasModernData = filtered.some(d => (d.YEAR === 2023 || d.YEAR === 2024) && d.AGE_GROUP.toLowerCase() !== "all ages");
        if (hasModernData) {
            filtered = filtered.filter(d => d.YEAR === 2023 || d.YEAR === 2024);
        }
    }
    ...
    // Omit default master aggregates when plotting distinct categorical distributions
    if (chartData.length > 1 && chartData.some(d => d.key.toLowerCase() === "all ages")) {
        chartData = chartData.filter(d => d.key.toLowerCase() !== "all ages");
    }

    // Sort chart data according to the predefined array configuration order
