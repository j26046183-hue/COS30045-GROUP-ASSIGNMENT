// =====================
// DRUG-TESTS.JS (FULLY INTEGRATED STATE + YEAR CROSS-FILTERING)
// 4 Charts Summary Grid:
// 1. Historical trend (Timeline 2008-2024) — Listens to State (Year filter ignored so lines form)
// 2. Drug type breakdown (Donut) — Listens to State + Year 🚀 (Upgraded!)
...
function applyDrugFilters() {
    ...
    // Now perfectly filters on BOTH State and Year because of your new column! 🚀
...
// ── CHART 2: Drug Type Donut (State + Year Dynamic Breakdown) ──
function drawDrugDonut(data) {
    ...
    const margin = { top: 10, right: 10, bottom: 85, left: 10 };
    ...
    if (total === 0) {
        svg.append("text").attr("text-anchor", "middle")
            ...
            .attr("font-family", "DM Sans, sans-serif").text("No Data for Selected Filter");
        return;
    }
    ...
    svg.selectAll(".arc")
        ...
        .on("mouseover", function(event, d) {
            ...
            drugTooltip.style("opacity", 1)
                .html(`<strong>${drugLabels[d.data.drug]}</strong><br>Count: ${d.data.value.toLocaleString()}<br>Share: ${pct}%`);
        })
        .on("mousemove", function(event) {
            drugTooltip.style("left", (event.pageX + 14) + "px").style("top", (event.pageY - 32) + "px");
        })
    ...
    svg.append("text").attr("text-anchor", "middle").attr("dy", "1.2em")
        ...
        .attr("font-family", "DM Sans, sans-serif").text("Detections");
