...
// Toggle and drawer are separate direct children of <body> so each
// gets its own fixed stacking context — no z-index interference.
function buildNotesPanel() {
    ...
    toggle.innerHTML = `
        ...
        <span id="notes-toggle-label">Data Details</span>
    `;
    ...
    `;
    drawer.querySelector('#notes-close').addEventListener('click', toggleNotes);
...
window.switchPage = function(pageId) {
    ...
    renderNotes(pageId);
    // Close the drawer on page switch
    if (notesOpen) {
        notesOpen = false;
        const drawer = document.getElementById('notes-drawer');
        const toggle = document.getElementById('notes-toggle');
