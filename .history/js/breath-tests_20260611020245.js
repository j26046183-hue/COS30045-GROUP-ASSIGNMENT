...
Promise.all([
    ...
    const uniqueYears = [...new Set(data.map(d => d.YEAR))].filter(y => y).sort((a, b) => b - a);
...
function applyBreathFilters() {
    ...
    const barYear = d3.select("#bar-year-filter").property("value") || "all";
    ...
    let stateFiltered = breathMasterData.slice();
    if (selectedState !== "all") {
        stateFiltered = stateFiltered.filter(d => d.STATE === selectedState);
    }
    ...
    drawBreathHistorical(stateFiltered);
...
function updateDonutOnly() {
    const selectedState = d3.select("#breath-state-filter").property("value") || "all";
    const selectedYear = d3.select("#donut-year-filter").property("value") || "all"; // Local donut selection
    ...
    drawBreathRegionalDonut(filtered, selectedState, selectedYear);
}

function updateBarOnly() {
    const selectedState = d3.select("#breath-state-filter").property("value") || "all";
    const selectedYear = d3.select("#bar-year-filter").property("value") || "all"; // Local bar selection
    ...
    renderBreathBarChart(filtered, selectedState, selectedYear);
}
