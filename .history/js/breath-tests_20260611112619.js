...
function drawBreathRegionalDonut(data) {
    ...
    const height = 320;
    // Increased bottom margin slightly to give your centered legend room to breathe
    ...
    // Aggregate counts and normalize strings (e.g., "Major Cities of Australia" -> "Major Cities")
    ...
    // Fail-safe UI block if the selected state contains no data rows
    if (totalVolume === 0) {
        chartG.append("text").attr("text-anchor", "middle")
            .attr("font-size", "13px").attr("fill", "#94a3b8")
            .attr("font-family", "DM Sans, sans-serif").text("No Location Data Reported for Selection");
        return;
    }
    ...
    // Dynamic grid layout for Legend labels
    const legendG = svg.append("g");
    let cx = 0, cy = 0;
    const sx = 140, sy = 18;
    
    // Only display categories in the legend that actually contain data values (> 0)
    totals.filter(d => d.value > 0).forEach((d, i) => {
        ...
        if ((i + 1) % 2 === 0) { cx = 0; cy += sy; } else { cx += sx; }
    });
