...
const breathAgeColors = ["#cbd5e1", "#bfdbfe", "#60a5fa", "#2563eb", "#1d4ed8", "#475569", "#94a3b8"];

let breathMasterData = [];
let breathHistoricalData = [];
let breathModernData     = [];

// ── LOAD UNIFIED DATA WITH AUTO-CORRECT REPAIR ENGINE ──
d3.csv("data/breath_2023_2024.csv").then(function(rawCSV) {
    ...
    rawCSV.forEach(d => {
        // 1. Clean casing whitespace flaws (e.g. "tas " or "Tas" -> "TAS")
        if (d.STATE) d.STATE = d.STATE.trim().toUpperCase();
        
        // 2. Structural normalization: If values are empty/blank, inject structural defaults
        if (!d.LOCATION || d.LOCATION.trim() === "") d.LOCATION = "Location Not Specified";
        if (!d.AGE_GROUP || d.AGE_GROUP.trim() === "") d.AGE_GROUP = "Age Group Not Specified";

        // 3. Prevent NaN arithmetic from choking scale domains
        d.YEAR           = +d.YEAR;
        d.COUNT          = +d.COUNT          || 0;
    ...
    // Pass everything to the historical tracker line chart
    breathHistoricalData = breathMasterData; 
    
    // Explicit modern slice for KPIs, Donut, and Bar charts
    breathModernData = breathMasterData.filter(d => d.YEAR === 2023 || d.YEAR === 2024);

    // Build unique selection dropdown values dynamically
...
function applyBreathFilters() {
    ...
    // Dom population formatting anchors
...
// ══════════════════════════════════════════
// CHART 1: HISTORICAL LINE CHART (Calculates Any Formatting Type)
// ══════════════════════════════════════════
function drawBreathHistorical(data) {
    d3.select("#breath-historical-chart").selectAll("*").remove();
    if (!data || data.length === 0) return;

    // Direct aggregation collapse strategy to handle macro and micro data rows for TAS
...
function drawBreathRegionalDonut(data, selectedState) {
    ...
    const totals = Array.from(locMap, ([region, value]) => ({ region, value })).filter(d => d.value > 0);
    const totalVolume = d3.sum(totals, d => d.value);

    if (totalVolume === 0) {
        chartG.append("circle").attr("r", radius).attr("fill", "none").attr("stroke", "#e2e8f0").attr("stroke-width", 28).attr("stroke-dasharray", "8,6");
        chartG.append("text").attr("text-anchor", "middle").attr("dy", "0.4em").attr("font-size", "12px").attr("fill", "#94a3b8").text("No Spatial Volume Registered");
        return;
    }
...
function drawBreathAgeBar(data, selectedState) {
    ...
    if (data.length === 0) {
        svg.append("text").attr("x", width / 2).attr("y", height / 2).attr("text-anchor", "middle").attr("font-size", "13px").attr("fill", "#94a3b8").text("No demographic records matching filters");
        return;
    }

    const ageMap = d3.rollup(data.filter(d => d.AGE_GROUP), v => d3.sum(v, d => d.COUNT), d => d.AGE_GROUP);
    const chartData = Array.from(ageMap, ([key, value]) => ({
        key,
        value,
    ...
    if (chartData.length === 0) {
        svg.append("text").attr("x", width / 2).attr("y", height / 2).attr("text-anchor", "middle").attr("font-size", "12px").attr("fill", "#94a3b8").text("No granular age breakdown metrics returned");
        return;
    }
