...
Promise.all([
    ...
    if (donutYearSelect.node()) {
        donutYearSelect.selectAll("option:not([value='all'])").remove();
    ...
    if (barYearSelect.node()) {
        barYearSelect.selectAll("option:not([value='all'])").remove();
...
function applyBreathFilters() {
    ...
    // Update mini stats
    const totalTests   = d3.sum(filteredHistorical.filter(d => d.YEAR >= 2023), d => d.COUNT);
...
function drawBreathRegionalDonut(data) {
    ...
    const totals = regions.map(region => {
        ...
        return { region, value: d3.sum(rows, d => d.COUNT) };
    });
...
function renderBreathBarChart(data) {
    ...
    const rolled = d3.rollups(data, v => d3.sum(v, d => d.COUNT), d => d.AGE_GROUP)
