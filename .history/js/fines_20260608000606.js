...
Promise.all([
    ...
    metricFilter.selectAll("option:not([value='all'])").remove();
...
function drawFinesHistorical(data) {
    ...
    // Get an array of all unique years in the data sorted chronologically
    ...
    // ── CHANGED PART: Force D3 to display every single year ──
    svg.append("g").attr("class", "axis").attr("transform", `translate(0,${height})`)
        .call(d3.axisBottom(x)
            .tickValues(uniqueYears)          // <-- Forces every year label to print
            .tickFormat(d3.format("d"))       // <-- Keeps format as simple digits (e.g., 2015)
        );
