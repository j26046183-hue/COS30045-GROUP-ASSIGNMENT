...
// BREATH-TESTS.JS
// 3 Charts:
// 1. Historical Line Chart   — breath_historical_trend.csv (state filter INSIDE chart box)
// 2. Regional Donut Chart    — breath_2023_2024.csv (only ACT, NSW, VIC — year filter)
// 3. Age Group Bar Chart     — breath_2023_2024.csv (only ACT, NSW, VIC — year filter)
// =====================
...
// States that have detailed breakdown data in breath_2023_2024.csv
const STATES_WITH_DETAIL = ["ACT", "NSW", "VIC"];
const STATES_NO_DETAIL   = ["NT", "QLD", "SA", "TAS", "WA"];
...
Promise.all([
    ...
    // ── Line chart state filter — all 8 states ──
    const allStates = [...new Set(historical.map(d => d.STATE))].sort();
    const lineStateFilter = d3.select("#breath-line-state-filter");
    if (lineStateFilter.node()) {
        lineStateFilter.selectAll("option:not([value='all'])").remove();
        allStates.forEach(s => lineStateFilter.append("option").attr("value", s).text(s));
        lineStateFilter.on("change", updateLineOnly);
    }

    // ── Donut state filter — only ACT, NSW, VIC ──
    const donutStateFilter = d3.select("#breath-donut-state-filter");
    if (donutStateFilter.node()) {
        donutStateFilter.selectAll("option:not([value='all'])").remove();
        STATES_WITH_DETAIL.forEach(s => donutStateFilter.append("option").attr("value", s).text(s));
        donutStateFilter.on("change", updateDonutOnly);
    }

    // ── Bar state filter — only ACT, NSW, VIC ──
    const barStateFilter = d3.select("#breath-bar-state-filter");
    if (barStateFilter.node()) {
        barStateFilter.selectAll("option:not([value='all'])").remove();
        STATES_WITH_DETAIL.forEach(s => barStateFilter.append("option").attr("value", s).text(s));
        barStateFilter.on("change", updateBarOnly);
    }

    // ── Year filters for donut and bar ──
    ...
    if (donutYearSelect.node()) {
        ...
        donutYearSelect.on("change", updateDonutOnly);
    }
    ...
    if (barYearSelect.node()) {
        ...
        barYearSelect.on("change", updateBarOnly);
    }

    // ── Mini stats from historical (all states 2023+2024) ──
    const hist2324     = historical.filter(d => d.YEAR >= 2023);
    ...
    const stateMap     = d3.rollup(historical, v => d3.sum(v, d => d.COUNT), d => d.STATE);
...
// ── LINE CHART FILTER ──
function updateLineOnly() {
    const node = d3.select("#breath-line-state-filter").node();
    const selectedState = node ? node.value : "all";
    let filtered = breathHistoricalData.slice();
    if (selectedState !== "all") filtered = filtered.filter(d => d.STATE === selectedState);
    drawBreathHistorical(filtered);
}
...
function updateBarOnly() {
    ...
    drawBreathAgeBar(filtered);
}


// ══════════════════════════════════════════
// CHART 1: HISTORICAL LINE CHART
// State filter is inside the chart box (breath-line-state-filter)
// ══════════════════════════════════════════
...
// Only ACT, NSW, VIC in state filter
// Note shown about other states
// ══════════════════════════════════════════
function drawBreathRegionalDonut(data) {
    ...
    if (totalVolume === 0) {
        chartG.append("circle")
            ...
            .attr("stroke", "#e2e8f0").attr("stroke-width", 28).attr("stroke-dasharray", "8,6");
        chartG.append("text").attr("text-anchor", "middle").attr("dy", "0.4em")
    ...
    chartG.selectAll(".arc")
        ...
        .attr("stroke", "white").attr("stroke-width", 2.5).style("cursor", "pointer")
        .on("mouseover", function(event, d) {
    ...
    // Center label
    chartG.append("text").attr("text-anchor", "middle").attr("dy", "-0.3em")
        .attr("font-family", "Syne, sans-serif").attr("font-size", "20px")
        .attr("font-weight", "700").attr("fill", "#0f172a")
    ...
    chartG.append("text").attr("text-anchor", "middle").attr("dy", "1.2em")
        ...
        .attr("font-family", "DM Sans, sans-serif").attr("letter-spacing", "0.8px")
        .text("TOTAL TESTS");

    // Legend
    const legendG  = svg.append("g");
    const itemW    = 130, itemH = 18, cols = 2;
    totals.forEach((d, i) => {
        const col = i % cols;
...
// Note shown about other states
// ══════════════════════════════════════════
function drawBreathAgeBar(data) {
    ...
    svg.selectAll(".bar").data(chartData).enter().append("rect")
        .attr("class", "bar").attr("x", d => x(d.key)).attr("width", x.bandwidth())
        .attr("y", height).attr("height", 0).attr("fill", d => d.color).attr("rx", 5)
        .on("mouseover", function(event, d) {
    ...
    svg.selectAll(".bar-label").data(chartData).enter().append("text")
        ...
        .attr("x", d => x(d.key) + x.bandwidth() / 2).attr("y", d => y(d.value) - 6)
        .attr("text-anchor", "middle").attr("font-size", "11px")
        .attr("fill", "#475569").attr("font-family", "DM Sans, sans-serif")
