// =========================================================================
// BREATH-TESTS.JS
// 3 Charts (Fully Relational, Optimized Clean Layout):
// 1. Historical Trend — HEATMAP MATRIX (States vs Years)
// 2. Total Positive Tests — CATEGORICAL SCATTER PLOT (Distributed Bubble Dispersion)
// 3. Enforcement Actions — PROPORTIONAL DONUT CHART
// =========================================================================
...
// =========================================================================
// CHART 2: TOTAL BREATH TESTS — FIXED CATEGORICAL SCATTER PLOT
// =========================================================================
function drawBreathScatterPlot(data) {
    ...
    // FIX: Spread States evenly across the X Axis to completely eliminate cluttering
    const states = ["ACT", "NSW", "NT", "QLD", "SA", "TAS", "VIC", "WA"];
    const x = d3.scalePoint().domain(states).range([40, width - 40]);
    
    // Y-Axis tracks the dynamic numeric Count cleanly
    const y = d3.scaleLinear().domain([0, d3.max(data, d => d.COUNT) * 1.2 || 100]).range([height, 0]);
    
    // Sqrt scale dynamically drives bubble radius sizing so they look premium and well proportioned
    const r = d3.scaleSqrt().domain([0, d3.max(data, d => d.COUNT) || 1]).range([10, 28]);
    ...
    // Plot Floating Scatter Bubble Nodes
    const nodes = svg.selectAll(".scatter-node")
        .data(data.filter(d => states.includes(d.STATE)))
        .enter().append("g")
    ...
    // Dynamic clean text labels placed directly overhead of each bubble node
    nodes.append("text")
        .attr("x", d => x(d.STATE))
        .attr("y", height)
        ...
        .style("fill", d => stateColors[d.STATE] || "#475569")
        .text(d => d.COUNT > 0 ? d.COUNT.toLocaleString() : "")
        .transition().duration(800)
        .attr("y", d => y(d.COUNT) - r(d.COUNT) - 8);
}
...
function drawBreathDonutChart(data) {
    ...
    ].filter(d => d.value > 0); // Hide 0 value slices cleanly
