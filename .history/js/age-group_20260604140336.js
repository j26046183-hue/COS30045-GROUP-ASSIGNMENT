...
const ageBarColors = ["#bfdbfe", "#60a5fa", "#2563eb", "#1d4ed8", "#1e3a8a"];
...
// Store raw data globally for resize
let ageRawData = [];
...
// Draw function — recalculates width every time
function drawAgeChart(data) {

    // Clear entire container and redraw svg fresh
    d3.select("#age-group-chart").selectAll("*").remove();
    ...
    ageSvg.append("g")
        .attr("class", "grid")
        .call(d3.axisLeft(y).ticks(5).tickSize(-ageWidth).tickFormat(""))
        .selectAll("line")
        .style("stroke", "#f1f5f9")
...
// Resize support
window.addEventListener("resize", applyAgeFilters);