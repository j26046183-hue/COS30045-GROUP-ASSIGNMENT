...
// ── LOAD UNIFIED DATA WITH AUTO-NORMALIZATION ──
...
function applyBreathFilters() {
    ...
    // Handle KPI summary cards cleanly if modern figures don't exist
    d3.select("#breath-total").text(totalCount > 0 ? totalCount.toLocaleString() : "Not Reported");
    d3.select("#breath-charges").text(totalCharges > 0 ? totalCharges.toLocaleString() : "Not Reported");
    d3.select("#breath-arrests").text(totalArrests > 0 ? totalArrests.toLocaleString() : "Not Reported");
...
function drawBreathHistorical(data) {
    ...
    const lineDataMap = d3.rollup(
        data.filter(d => d.LOCATION === "All regions" || d.LOCATION === "Location Not Specified"),
        v => d3.sum(v, d => d.COUNT),
        d => d.STATE,
        d => d.YEAR
    );
    ...
    lineDataMap.forEach((yearMap, state) => {
        ...
        yearMap.forEach((count, year) => {
            if (count > 0) { // Protect scale calculations from picking up flat zero-lines
...
// ══════════════════════════════════════════
// CHART 2: REGIONAL DONUT CHART (Elegantly Handles Missing State Data)
// ══════════════════════════════════════════
function drawBreathRegionalDonut(data, selectedState) {
    ...
    // Dynamic clean interface messaging for missing records (like TAS 2023-2024)
    if (totalVolume === 0) {
        chartG.append("circle").attr("r", radius).attr("fill", "none").attr("stroke", "#f1f5f9").attr("stroke-width", 14).attr("stroke-dasharray", "6,4");
        
        const stateName = selectedState === "all" ? "Australia" : selectedState;
        chartG.append("text").attr("text-anchor", "middle").attr("dy", "-0.2em").attr("font-size", "13px").attr("font-weight", "600").attr("fill", "#64748b").text("Granular metrics not reported");
        chartG.append("text").attr("text-anchor", "middle").attr("dy", "1.1em").attr("font-size", "12px").attr("fill", "#94a3b8").text(`for ${stateName} (2023-2024)`);
        return;
    }
...
// ══════════════════════════════════════════
// CHART 3: AGE GROUP BAR CHART (Elegantly Handles Missing State Data)
// ══════════════════════════════════════════
function drawBreathAgeBar(data, selectedState) {
    ...
    if (chartData.length === 0) {
        ...
        svg.append("text")
            ...
            .attr("fill", "#64748b")
            .text(`No demographic records available for ${stateName} (2023-2024)`);
        return;
    }
