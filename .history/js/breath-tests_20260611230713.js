...
function applyBreathFilters() {
    ...
    const stateMap = d3.rollup(
        breathMasterData.filter(d => d.LOCATION === "All regions" || d.LOCATION === "Location Not Specified"), 
...
function drawBreathRegionalDonut(data, selectedState) {
    ...
    if (totalVolume === 0) {
        ...
        chartG.append("text").attr("text-anchor", "middle").attr("dy", "-0.2em").attr("font-size", "13px").attr("font-weight", "600").attr("fill", "#64748b").text("No spatial records available");
        chartG.append("text").attr("text-anchor", "middle").attr("dy", "1.1em").attr("font-size", "12px").attr("fill", "#94a3b8").text(`for ${stateName}`);
        return;
    }
...
function drawBreathAgeBar(data, selectedState) {
    ...
    if (chartData.length === 0) {
        ...
        svg.append("text")
            ...
            .attr("fill", "#64748b")
            .text(`No demographic records available for ${stateName}`);
        return;
    }
