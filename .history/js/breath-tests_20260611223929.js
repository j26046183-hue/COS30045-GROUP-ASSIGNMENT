...
const regionalColors = {
    ...
    "Location Not Specified":     "#94a3b8" // Sleek slate gray for newly added state fallback tracking
};
...
// Expanded age configurations to completely support your new granular rows
...
function applyBreathFilters() {
    ...
    // Mini stats — calculate off modern data matrix to maintain consistency with granular parameters
    let filteredModernStats = breathModernData.slice();
    if (selectedState !== "all") {
        filteredModernStats = filteredModernStats.filter(d => d.STATE === selectedState);
    }
    
    const totalCount   = d3.sum(filteredModernStats, d => d.COUNT);
    const totalCharges = d3.sum(filteredModernStats, d => d["TOTAL CHARGES"]);
    const totalArrests = d3.sum(filteredModernStats, d => d["TOTAL ARRESTS"]);
...
function drawBreathRegionalDonut(data, selectedState) {
    ...
    // Added "Location Not Specified" to display options cleanly
    const locationOrder = [
    ...
    if (totalVolume === 0) {
        ...
        chartG.append("text")
            ...
            .attr("font-family", "DM Sans, sans-serif")
            .text("No Test Volume Registered");
        return;
    }
    ...
    // Legend — centered below donut grid
...
function drawBreathAgeBar(data, selectedState) {
    ...
    // Rollup by age group — filters out 'All ages' rows to prevent QLD from breaking scaling metrics
    ...
    // If counts are strictly 0 across categories (e.g., NT reporting patterns), fallback check metrics
    if (chartData.length === 0) {
        svg.append("text")
            .attr("x", width / 2).attr("y", height / 2)
            .attr("text-anchor", "middle").attr("font-size", "13px")
            .attr("fill", "#94a3b8").attr("font-family", "DM Sans, sans-serif")
            .text("No demographic test distributions available");
        return;
    }
    ...
    svg.append("g").attr("class", "axis")
        ...
        .attr("dy", "1.5em")
        .call(wrap, x.bandwidth()); // Added text wrapper utility to cleanly balance "Age Group Not Specified" labels
...
// Axis text wrap styling helper function
